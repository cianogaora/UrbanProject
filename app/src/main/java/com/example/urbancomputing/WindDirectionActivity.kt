...
class WindDirectionActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_wind_direction)
            ...
            for(i in 0..22 step 2){
                if(i < 10){
                    ...
                    ref.child("hourly-weather/$date/0$i:00:00").get().addOnSuccessListener {
                        it.children.associate { it.key.toString() to it.value.toString() }["wdir"]
                            ?.let {
                    ...
                    ref.child("hourly-weather/$date/$i:00:00").get().addOnSuccessListener {
                        ...
                        if(entries.size == 12){
                            val vl = LineDataSet(entries, "Wind Direction (degrees)")
