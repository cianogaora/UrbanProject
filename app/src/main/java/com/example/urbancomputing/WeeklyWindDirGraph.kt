...
class WeeklyWindDirGraph : AppCompatActivity() {
    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_weekly_wind_dir_graph)
            ...
            if(days.size == 7){
                ...
                for (j in 0..6 step 1) {
                    ref.child("daily-weather/${days.get(j)}").get().addOnSuccessListener {
                        ...
                        //Log.d("AirP", "$pres")
                        pres["wdir"]?.let { it1 -> Entry( (i - j).toFloat(), it1.toFloat()) }
                            ?.let { it2 -> entries.add(0, it2) }
                        Log.d("AirP", "$entries")
                        if(entries.size == 7){
                            val vl = LineDataSet(entries, "Wind Direction (°)")
