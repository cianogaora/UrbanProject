...
class WeeklyPrecipitationGraph : AppCompatActivity() {
    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_weekly_precipitation_graph)
            ...
            if(days.size == 7){
                ...
                for (j in 0..6 step 1) {
                    ref.child("daily-weather/${days.get(j)}").get().addOnSuccessListener {
                        ...
                        //Log.d("AirP", "$pres")
                        pres["prcp"]?.let { it1 -> Entry( (i - j).toFloat(), it1.toFloat()) }
